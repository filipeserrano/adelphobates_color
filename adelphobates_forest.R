...
names(br_states)
br_states <- br_states [, c("SIGLA_UF", "geometry")]
centroides_estados <- st_centroid(br_states)

sigla_states <- br_states %>%
  st_point_on_surface() %>%
  mutate(
    x= st_coordinates(.)[,1],
    y= st_coordinates(.)[,2]
...
sigla_states <- sigla_states %>%
 mutate(
   x = ifelse(SIGLA_UF == "AC", x + 0.1, x),
   y = ifelse(SIGLA_UF == "AC", y + 0.3, y)
   )
